...
void CreateBoxMon(struct BoxPokemon *boxMon, u16 species, u8 level, u8 fixedIV, u8 hasFixedPersonality, u32 fixedPersonality, u8 otIdType, u32 fixedOtId)
{
    ...
    if (VarGet(VAR_GENERATE_SHINY) > 0) // Value of VAR_GENERATE_SHINY will decide how many shiny Pokémon will be generated as the Player's next encounters.
    {
        do
        {
            personality = Random32();
...
// By default, only the HP IV and the lower 3 bits of the Attack IV are read, while the rest of them become 0.
// A fix for this bug was left here by the members of Pret, and I decided to use it because why not.
    ...
    case 0:
        ...
        for (i = 0; i < 5; i++)
        {
            ...
            case EVO_LEVEL_SPECIFIC_MON_TYPE_IN_PARTY:
                ...
                break;
            case EVO_HELD_ITEM_LV42:
                if (level >= 42 && heldItem == gEvolutionTable[species][i].param)
                {
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                }
                break;
